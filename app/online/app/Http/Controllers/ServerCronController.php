...
class ServerCronController extends Controller
{
    ...
    public function __createProspect($data){        
        $contact = Contacts::create([
            ...
            "mnumber" => $this->__NumberFormater(array_pop($data["attributes"]['mobilePhones'])),
            "hnumber" => $this->__NumberFormater(array_pop($data["attributes"]['homePhones'])),
            "wnumber" => $this->__NumberFormater(array_pop($data["attributes"]['workPhones'])),
        ...
        $this->updateAllProspectsBack($contact->id);
    }
    ...
    //     return $string;
    // }
    public function updateAllProspectsBack($id = null){
        ...
        $record->number1 = $this->__NumberFormater($mobilePhones);
        $record->number2 = $this->__NumberFormater($workPhones);
        $record->number3 = $this->__NumberFormater($homePhones);
        $record->number1type = 'M';
        $record->number2type = 'W';
        $record->number3type = 'H';
        $record->number1call = $this->__NumberType($this->__NumberFormater($mobilePhones), $record->record_id);
        $record->number2call = $this->__NumberType($this->__NumberFormater($workPhones), $record->record_id);
        $record->number3call = $this->__NumberType($this->__NumberFormater($homePhones), $record->record_id);
    ...
    public function __updateProspect($data){
        ...
        $prospect->mnumber = $this->__NumberFormater(array_pop($data["attributes"]['mobilePhones']));
        $prospect->hnumber = $this->__NumberFormater(array_pop($data["attributes"]['homePhones']));
        $prospect->wnumber = $this->__NumberFormater(array_pop($data["attributes"]['workPhones']));
    ...
    public function getOutreachRecordsTags($i){ 
        ...
        //for($i = 0; $i < 45; $i++){
            ...
            foreach($results['data'] as $key => $value){
                $attributes = get_object_vars($value->attributes);
                    ...
                    DB::table('outreach_tags')->insert([
                        "record_id" => $value->id,
                        "outreach_tags" => implode(',', $attributes['tags']),
                    ]);
                        
            }
        //}
        return view('csyncb', compact('i'));
    }
    ...
    public function getOutreachMailing(){
        ...
        $count = $meta["count"];// echo $count; die;
        if($count > 0){
        ...
        for($i = 0; $i < $count; $i++){
            if($contact = TempEmails::count() > 0){
                ...
                if($contact->record_id > 0){
                    $totalemail = OutreachMailings::where('contact_id', $contact->record_id)->whereNotNull('deliveredAt')->count();
                    $totalopen = OutreachMailings::where('contact_id', $contact->record_id)->whereNotNull('openedAt')->count();
                    $totalreply = OutreachMailings::where('contact_id', $contact->record_id)->whereNotNull('repliedAt')->count();
                    $totalbounced = OutreachMailings::where('contact_id', $contact->record_id)->whereNotNull('bouncedAt')->count();
                    $totalclick = OutreachMailings::where('contact_id', $contact->record_id)->whereNotNull('clickedAt')->count();
        ...
        if(isset($return["envBody"]["ns2getReportResultResponse"]["return"]["records"])):
            ...
            foreach(explode("\n",$returnCsv["envBody"]["ns2getReportResultCsvResponse"]["return"]) as $key => $value){
                ...
                if($key > 0 && (count($value) == count($headerArray))){
                    ...
                    if( intval($records["record_id"]) > 0 && intval($records["dnis"]) > 0){
                        ...
                        $dnis = $records["dnis"];
                        $number1 = $records["number1"];
                        $number2 = $records["number2"];
                        $number3 = $records["number3"];
                        $time = explode(":", $talk_time);
                        $seconds = 0;
                        if(isset($time[0]) && intval($time[0]) > 0){ // hours
                            $seconds = $seconds+intval($time[0])*3600;
                        }
                        if(isset($time[1]) && intval($time[1]) > 0){ //minute
                            $seconds = $seconds+intval($time[1])*60;
                        }
                        if(isset($time[2]) && intval($time[2]) > 0){ // seconds
            ...
            if($contact = TempCalls::count() > 0){
                ...
                if($counts == 0){
                    ...
                    FivenineCallLogs::create([
                        ...
                        "queue_wait_time" => $contact->queue_wait_time,
                    ]);
                    $ucontact = Contacts::where("record_id", "=", $contact->record_id)->first();
                    if($ucontact){
                        $totalmattempt = FivenineCallLogs::where('record_id', $ucontact->record_id)->where('dial_attempts', '>=', 1)->where('dnis', $ucontact->number1)->sum('dial_attempts');
                        $totalhattempt = FivenineCallLogs::where('record_id', $ucontact->record_id)->where('dial_attempts', '>=', 1)->where('dnis', $ucontact->number3)->sum('dial_attempts');
                        $totaldattempt = FivenineCallLogs::where('record_id', $ucontact->record_id)->where('dial_attempts', '>=', 1)->where('dnis', $ucontact->number2)->sum('dial_attempts');
            
                        $totalmreceived = FivenineCallLogs::where('record_id', $ucontact->record_id)->where('call_received', 1)->where('dial_attempts', '>=', 1)->where('dnis', $ucontact->number1)->count();
                        $totalhqreceived = FivenineCallLogs::where('record_id', $ucontact->record_id)->where('call_received', 1)->where('dial_attempts', '>=', 1)->where('dnis', $ucontact->number3)->count();
                        $totaldreceived = FivenineCallLogs::where('record_id', $ucontact->record_id)->where('call_received', 1)->where('dial_attempts', '>=', 1)->where('dnis', $ucontact->number2)->count();
    ...
    public function croneAgentOccupancy(){
        ...
        curl_setopt_array($curl, array(
            ...
            // dd($returnCsv["envBody"]["ns2getReportResultCsvResponse"]["return"]);
            foreach(explode("\n",$returnCsv["envBody"]["ns2getReportResultCsvResponse"]["return"]) as $key => $value){
                $value = str_getcsv($value, ",", '"');
                if($key > 0 && (count($value) == count($headerArray))){
                    $records = array_combine($headerArray,$value);
                    ...
                    if($count == 0){
                        ...
                        $data["name"] = ucfirst($records["agent_first_name"]).' '.ucfirst($records["agent_last_name"]);
