...
use App\Models\ProspectCustoms;
...
class ServerCronLatestController extends Controller
{
    ...
    public function __createContact($data){        
    ...
    public function __createProspect($data){        
        ...
        ProspectCustoms::create($dataC);
    }
    public function __updateProspect($contact, $contactCustom, $data){
        ...
        $prospect->country = $contact['addressCountry'];
        $prospect->city =  $contact['addressCity'];
        $prospect->state =  $contact['addressState'];
        $prospect->zip =  $contact['addressZip'];
        $prospect->address =  $contact["addressStreet"]." ". $contact["addressStreet2"];
        $prospect->company =  $contact['company'];
        $prospect->companyIndustry =  $contact['companyIndustry'];
        $prospect->emails = $contact['emails'];
        $prospect->engage_score =   $contact['engagedScore'];
        $prospect->first_name =   $contact['firstName'];
        $prospect->last_name =   $contact['lastName'];
        $prospect->name =  $contact['firstName']." ". $contact['lastName'];
        $prospect->mobilePhones = $contact['mobilePhones'];
        $prospect->workPhones = $contact['workPhones'];
        $prospect->homePhones = $contact['homePhones'];
        $prospect->otherPhones = $contact['otherPhones'];
        $prospect->voipPhones = $contact['voipPhones'];
        $prospect->mnumber = $contact['mobilePhones'];
        $prospect->hnumber = $contact['homePhones'];
        $prospect->wnumber = $contact['workPhones'];
        $prospect->linkedInUrl =  $contact['linkedInUrl'];
        $prospect->source =  $contact['source'];
        $prospect->title =  $contact['title'];
        $prospect->account_id = $contact["account_id"];
        $prospect->outreach_tag = $contact['tags'];
        $prospect->timeZone =  $contact["timeZone"];
        $prospect->outreach_touched_at =  $contact['touchedAt'];
        $prospect->last_update_at =  $contact['updatedAt'];
        $prospect->websiteUrl1 =  $contact['websiteUrl1'];
        $prospect->last_outreach_engage =  $contact['engagedAt'];
        $prospect->stage = intval($data["stage"]);
        $prospect->last_outreach_email =  $contact['emailsOptedAt'];
        $prospect->outreach_created_at =  $contact['createdAt'];
        $prospect->outreach_persona = $contact["outreach_persona"];
        $prospect->custom1 =  $contact["custom1"];
        $prospect->custom2 =  $contact["custom2"];
        $prospect->custom9 =  $contact["custom9"];
        $prospect->custom10 =  $contact["custom10"];
        $prospect->custom11 =  $contact["custom11"];
        $prospect->custom12 =  $contact["custom12"];
        ...
        for($i = 1 ; $i < 151; $i++){
            if(($i != 30) && ($i != 31) && ($i != 32) && ($i != 24)){
                $custome["custom".$i] = $contactCustom["custom".$i];
            }
        }
        if($customeRecocord):
    ...
    { 
            ...
            for ($i=0; $i < $page; $i++) { 
                ...
                if(is_null($responseq) || is_null(json_decode($responseq)) || is_null(get_object_vars(json_decode($responseq)))){
                    ...
                    foreach($results['data'] as $key => $value){ 
                        ...
                        $count = Prospects::where('record_id', '=', $value->id)->count();
                        if($count == 0){
                            $allRecords[] = $value->id;
                            $data = [
                                "record_id" => $value->id,
        ...
        //         "total_stage_update_ids" => json_encode([]),
        //         "total_contact_no_update" => 0,
        //         "total_contact_no_update_ids" => json_encode([]),
        //         "total_custom_field_update" => 0,
        //         "total_custom_field_update_ids" => json_encode([])
        //     ]);
        // }
        $this->createJob();
    ...
    public function createJob(){
        $c = Prospects::count();
        for($i = 0; $i <= intval($c/200)+1; ++$i){
            MyTestJob::dispatch(["listname" => 'listName'])->onConnection('database');
        }
    }
    public function prospectCounter(){
        $createdContacts = 0;
        $createdContactsArray = [];
        $stageUpdate = 0;
        $stageUpdateArray = [];
        ...
        for($i = 0; $i < $count; $i++){
            if($contact = Prospects::count() > 0){
                $contact = Prospects::first();
                $contactCustom = ProspectCustoms::where('contact_id', '=', $contact->record_id)->first();
                $oldStateStatus = 0;
                if($this->__checkStageUpdate($contact->record_id, $contact->stage)){
                    $stageUpdateArray[] = $contact->record_id;
                    $oldStateStatus = 1;
                    ++$stageUpdate;
                }
                $contactNumbers = [
                    ...
                    "company_hq_number" => $contact["custom5"],
                ];
                if($this->__checkContactNumberUpdate($contact->record_id,$contactNumbers)){
                    $contactNoUpdateArray[] = $contact->record_id;
                    ++$contactNoUpdate;        
                }
                if($this->__checkCustomFieldUpdate($contact->record_id, $contactCustom)){
                    $customFieldUpdateArray[] = $contact->record_id;
                    ++$customFieldUpdate;
                }
                $data = [
                ...
                $this->__updateProspect($contact, $contactCustom, $data);
                DB::table("prospects")->where("record_id", '=', $contact->record_id)->delete();
                DB::table("prospect_customs")->where("contact_id", '=', $contact->record_id)->delete();
            }
        }
        $date = date("Y-m-d", strtotime("-1 day"));
        $date = '2021-12-08';
        ...
        if($count == 0){
            //echo 1;
            ReportContacts::create([
                "total_stage_update" => count($stageUpdateArray),
                "total_stage_update_ids" => json_encode($stageUpdateArray),
                "total_contact_no_update" => count($contactNoUpdateArray),
                "total_contact_no_update_ids" => json_encode($contactNoUpdateArray),
                "total_custom_field_update" => count($customFieldUpdateArray),
                "total_custom_field_update_ids" => json_encode($customFieldUpdateArray)
            ]);
        }else{
            //echo 2;
            $record = ReportContacts::where("date", '=', $date)->first();
            
            $stageUpdateArrayOld = json_decode($record->total_stage_update_ids);
            $stageUpdateArrayNew = $stageUpdateArray;
            $stageUpdateArrayMerge = array_merge($stageUpdateArrayOld, $stageUpdateArrayNew);
            $stageUpdateArrayUnique = array_unique($stageUpdateArrayMerge);
            
            $customFieldUpdateArrayOld = json_decode($record->total_custom_field_update_ids);
            $customFieldUpdateArrayNew = $customFieldUpdateArray;
            $customFieldUpdateArrayMerge = array_merge($customFieldUpdateArrayOld, $customFieldUpdateArrayNew);
            $customFieldUpdateArrayUnique = array_unique($customFieldUpdateArrayMerge);
            
            $contactNoUpdateArrayOld = json_decode($record->total_contact_no_update_ids);
            $contactNoUpdateArrayNew = $contactNoUpdateArray;
            $contactNoUpdateArrayMerge = array_merge($contactNoUpdateArrayOld, $contactNoUpdateArrayNew);
            $contactNoUpdateArrayUnique = array_unique($contactNoUpdateArrayMerge);
            
            
            $record = ReportContacts::where("date", '=', $date)->first();
            $record->total_stage_update = count($stageUpdateArrayUnique);
            $record->total_stage_update_ids = json_encode($stageUpdateArrayUnique);
            $record->total_contact_no_update = count($contactNoUpdateArrayUnique);
            $record->total_contact_no_update_ids = json_encode($contactNoUpdateArrayUnique);
            $record->total_custom_field_update = count($customFieldUpdateArrayUnique);
            $record->total_custom_field_update_ids = json_encode($customFieldUpdateArrayUnique);
            $record->save();
        }
    }
