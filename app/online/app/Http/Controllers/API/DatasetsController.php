...
class DatasetsController extends Controller
{
    ...
    public function getRecordContainerInfoCall(Request $request){
        ...
        foreach($ids as $id){
            $record = Contacts::where("id", "=", $id)->first();
            if($record->mobilePhones != '' || $record->workPhones !== '' || $record->homePhones != ''){
                if(in_array($record->number1, $called)):
                    $calledno = $record->number1;
                    $calledtype = 'm';
                elseif(in_array($record->number2, $called)):
                    $calledno = $record->number2;
                    $calledtype = 'w';
                elseif(in_array($record->number3, $called)):
                    $calledno = $record->number3;
                    $calledtype = 'h';
                else:
                    $calledno = '';
                    $calledtype = 'o';
