...
class DashboardCallController extends Controller
{
    ...
    public function getNumberBasedData(Request $request)
    {
        ...
        $number1 = FivenineCallLogs::whereBetween("n_timestamp", [$stime, $etime])
                    ->whereColumn('dnis', '=', 'number1')
                    ->when($agentName, function($query, $agentName){
                        if($agentName == 'None'){
                            $query->whereNull('agent_name');
                        }else{
                            $query->where('agent_name', 'like', $agentName);
                        }
                    })
                    ->when($list_name, function($query, $list_name){
                        $query->where('list_name', 'like', ($list_name == 'Manual')?'':$list_name);
                    })
                    ->get()->count();
        $number2 = FivenineCallLogs::whereBetween("n_timestamp", [$stime, $etime])
                    ->whereColumn('dnis', '=', 'number2')
                    ->whereColumn('dnis', '!=', 'number1')
                    ->when($agentName, function($query, $agentName){
                        if($agentName == 'None'){
                            $query->whereNull('agent_name');
                    ...
                    ->get()->count();
        $number3 = FivenineCallLogs::whereBetween("n_timestamp", [$stime, $etime])
                    ->whereColumn('dnis', '=', 'number3')
                    ->whereColumn('dnis', '!=', 'number1')
                    ->whereColumn('dnis', '!=', 'number2')
                    ->when($agentName, function($query, $agentName){
                        if($agentName == 'None'){
    ...
    function generateAgentExport(Request $request){
        $gt = $this->__getTimeInStrtottime($request->input("dateRange"));
        $stime = $gt["stime"];
        $etime = $gt["etime"];
        $data = [];
        ...
        $agentOccupandyDataExport = $request->input('agentOccupandyDataExport');
        foreach($sAgentCallStatus as $key => $value){
            if($value == 'None'){
                $data[$key] = [
                    "agent_name" => 'Auto Dial',
                    "calls" => FivenineCallLogs::whereNull('agent_name')->whereBetween("n_timestamp", [$stime, $etime])->count(),
                    "login_time" => 'NA',
                    "acw" => 'NA',
                    "talk_time" => 'NA',
                    "not_ready" => 'NA'
                ...
                $data[$key] = [
                    "agent_name" => $value,
                    "calls" => FivenineCallLogs::where('agent_name', 'like', $value)->whereBetween("n_timestamp", [$stime, $etime])->count(),
                    "login_time" => $this->__getHrs(intval($agentInfo['login_time_sum'])),
                    "acw" => $this->__getHrs(intval($agentInfo['on_acw_time_sum'])),
                    "talk_time" => $this->__getHrs(intval($agentInfo['on_call_time_sum'])),
                    "not_ready" => $this->__getHrs(intval($agentInfo['not_ready_time_sum']))
                ];
            }
        }
        return $data;
    }
    public function generateDispositionExport(Request $request){
        $gt = $this->__getTimeInStrtottime($request->input("dateRange"));
        $stime = $gt["stime"];
        $etime = $gt["etime"];
        $list_name = $request->input('list_name');
        $dispositions = FivenineCallLogs::when($list_name, function($query, $list_name){
            $query->where('list_name', 'like', $list_name);
        })
        ->select('disposition')->distinct()->get()->pluck('disposition')->toArray();
        
        $agentCallStatus = $this->getAgentCallStatus($request);
        $agentsName = $agentCallStatus['agentName'];
        
        $disData = [];
        $json_fields = [];
        $json_fields['Row Labels'] = 'All Dispositions';
        $json_fields['Auto Dispose'] = 'Auto Dispose';
        foreach($dispositions as $key => $disposition){
            $disData[$key] = [
                'All Dispositions' => $disposition,
                'Auto Dispose' => DB::table('fivenine_call_logs')
            ...
            foreach($agentsName as $agentName){
                if($agentName == 'None'){

                }else{
                    if($key == 0){
                        $json_fields[$agentName] = $agentName;
                    }
                    $disData[$key][$agentName] = DB::table('fivenine_call_logs')
        ...
        return ['json_fields' => $json_fields, 'json_data' => $disData];
    }
    ...
    public function getRecordContainerInfoCallA(Request $request)
    {
        $ids = Contacts::whereIn('id', $request->exports)->pluck('record_id')->toArray();
        $called = $request->called_numbers;
        $allRecords = [];
        foreach($ids as $id){
            $contact = Contacts::where('record_id', $id)->first();
            
            $recordIds = FivenineCallLogs::where('fivenine_call_logs.record_id', $id)->whereIn('dnis', $called);
            ...
            $recordIds = $recordIds->select('fivenine_call_logs.*', 'contacts.company', 'contacts.title')
    ...
    public function getRecordContainerInfoCall(Request $request)
    {
        ...
        $recordIds = $recordIds->select('fivenine_call_logs.*', 'contacts.company', 'contacts.title', 'contacts.first_name', 'contacts.last_name')
                            ->join('contacts', 'contacts.record_id', '=', 'fivenine_call_logs.record_id')->orderBy('fivenine_call_logs.record_id', 'asc')->get();
        return $recordIds;
    }
