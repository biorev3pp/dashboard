...
class ReportController extends Controller
{
    ...
    private function __getEmailSequenceLog($startDate, $endDate, $logRecordType){
        if($logRecordType == 1): //1:'Total Delivered'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->where("state", '=', "delivered")->WhereDate("deliveredAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->where("state", '=', "delivered")->WhereBetween("deliveredAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 2): //2:'Total Opened'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->where("state", '=', "opened")->WhereDate("deliveredAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->where("state", '=', "opened")->WhereBetween("deliveredAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 3): //3:'Total Clicked'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->WhereDate("clickedAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->WhereBetween("clickedAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 4): //4:'Total Replied'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->where("state", '=', "replied")->WhereDate("deliveredAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->where("state", '=', "replied")->WhereBetween("deliveredAt", [$startDate, $endDate])->get();
            endif;
            elseif($logRecordType == 5): //5:'Total Bounced'
                if($startDate == $endDate): // same day
                    return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->where("state", '=', "bounced")->WhereDate("bouncedAt", '=', $startDate)->get();
                else:
                    return OutreachMailings::with('prospect')->where("mailingType", "=", "sequence")->where("state", '=', "bounced")->WhereBetween("bouncedAt", [$startDate, $endDate])->get();
                endif;
        endif;
    }
    private function __getEmailSingleLog($startDate, $endDate, $logRecordType){        
        if($logRecordType == 1): //1:'Total Delivered'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->where("state", '=', "delivered")->WhereDate("deliveredAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->where("state", '=', "delivered")->WhereBetween("deliveredAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 2): //2:'Total Opened'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->where("state", '=', "opened")->WhereDate("deliveredAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->where("state", '=', "opened")->WhereBetween("deliveredAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 3): //3:'Total Clicked'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->WhereDate("clickedAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->WhereBetween("clickedAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 4): //4:'Total Replied'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->where("state", '=', "replied")->WhereDate("deliveredAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->where("state", '=', "replied")->WhereBetween("deliveredAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 5): //5:'Total Bounced'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->where("state", '=', "bounced")->WhereDate("bouncedAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("mailingType", "=", "single")->where("state", '=', "bounced")->WhereBetween("bouncedAt", '=', [$startDate, $endDate])->get();
            endif;
        endif;
    }
    private function __getEmailAllLog($startDate, $endDate, $logRecordType){        
        if($logRecordType == 1): //1:'Total Delivered'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("state", '=', "delivered")->WhereDate("deliveredAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("state", '=', "delivered")->WhereBetween("deliveredAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 2): //2:'Total Opened'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("state", '=', "opened")->WhereDate("deliveredAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("state", '=', "opened")->WhereBetween("deliveredAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 3): //3:'Total Clicked'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->WhereDate("clickedAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->WhereBetween("clickedAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 4): //4:'Total Replied'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("state", '=', "replied")->WhereDate("deliveredAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("state", '=', "replied")->WhereBetween("deliveredAt", [$startDate, $endDate])->get();
            endif;
        elseif($logRecordType == 5): //5:'Total Bounced'
            if($startDate == $endDate): // same day
                return OutreachMailings::with('prospect')->where("state", '=', "bounced")->WhereDate("bouncedAt", '=', $startDate)->get();
            else:
                return OutreachMailings::with('prospect')->where("state", '=', "bounced")->WhereBetween("bouncedAt", [$startDate, $endDate])->get();
            endif;
        endif;
    }
