                ...
                <ul role="tablist">
                    <li role="tab" class="first" :class="[(done_steps.indexOf(1) >= 0)?'done':(active_step == 1)?'current':'']">
                        <a id="steps-1" href="javascript:;">
                            ...
                            <b>{{ recordCount }} </b> Records Found (calls)
                        </a>
                    </li>
                    ...
                    <div class="divtbody custom-height">
                        <div class="divtbody-row" v-for="(record, rkey) in records" :key="'all-'+rkey">
                            ...
                            <div class="divtbody-elem wf-150 bg-dark1">
                                <b class="d-inline-block position-relative badge badge-dark wf-25" style="top:-2px" v-if="record.dnis == record.number1">N1</b>
                                <b class="d-inline-block position-relative badge badge-dark wf-25" style="top:-2px" v-else-if="record.dnis == record.number2">N2</b>
                                <b class="d-inline-block position-relative badge badge-dark wf-25" style="top:-2px" v-else>N3</b>
                ...
                <div v-else>
                    ...
                    <div class="divtable border-top" v-if="recordCount >= 1">
                        ...
                        <div class="divtbody custom-height-220 divtbody-refined" v-if="refinedview == true">
                            <div class="divtbody-row" v-for="(record, rkey) in form.records" :key="'all-'+rkey" :class="[ (highlightRecordIds.length  > 0 && highlightRecordIds.indexOf(record.record_id) > -1) ? 'op3' : (duplicateNumbers.indexOf(record.dnis)%2 == 0)?'op1':(duplicateNumbers.indexOf(record.dnis)%2 == 1)?'op2':'']">
...
export default {
    ...
    computed: {
        ...
        allRecordIds() {
            ...
            rdata.forEach((ele) => {
                ...
                if(ids.indexOf(ele.record_id) == -1){
                    ids.push(ele.record_id)
                }
            })
            return ids
        },
        records() {
            ...
            let refined_ids = [];
            rdata.forEach((ele) => {
                if(refined_ids.indexOf(ele.record_id + ele.dnis) == -1) {
                    refined_ids.push(ele.record_id + ele.dnis);
                    ...
                    if(ele.dnis == ele.number1){
                        fdata.ncount = 1
                    } else if(ele.dnis == ele.number2){
                        fdata.ncount = 2
                    } else {
                        fdata.ncount = 3
                    }
                    ...
                    let orkey = refined_ids.indexOf(ele.record_id + ele.dnis)
                    let cdata = new Object();
                    cdata.dial_attempts = ele.dial_attempts
                    cdata.timestamp = ele.timestamp
                    cdata.list_name = ele.list_name
    ...
    methods: {
            ...
            if(this.active_step == 3) {
                ...
                if(this.form.action == 'delete') {
                    ...
                    if(this.refinedview == true) {
                        if( (this.continue == false && this.allRecordIds.length - this.prospectSelected.length) > 0)
                            {
                                this.continue = false
                                this.highlightRecords = false
                                var diff = Number(this.allRecordIds.length - this.prospectSelected.length)
                                this.$swal({
                                    title: diff + ' prospect(s) not selected',
                                    //text: "You won't be able to revert this!",
                                    icon: 'warning',
                                    showDenyButton: true,
                                    denyButtonText : 'Highlight records',
                                    ...
                                    if (result.isConfirmed) {
                                        ...
                                        this.highlightRecords = true
                                        this.highlightRecordIds = this.allRecordIds.filter(x => !this.prospectSelected.includes(x));
        ...
        refinedContacts() {
            ...
            this.selectedRecords.filter((ele) => {
                if(this.prospectSelected.indexOf(ele.record_id) == -1){
                    this.prospectSelected.push(ele.record_id)
                }
            })
        },
        nextStep() {
