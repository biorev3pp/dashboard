            ...
            <div class="filterbox">
                <div class="row m-0">
                    <div class="col-md-4 col-12 pl-0">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-dark" @click="showView=true; filter = false" v-else>Default View</button>
                        
                        <a href="javascript:void(0)" class="link-primary" @click="showViewPopup"><i class="bi bi-plus"></i> Save View</a>
                        <div class="selection-box" v-if="showView">
                            <i class="bi bi-x close" @click="showView = false; filter = false"></i>
                            <label class="control-label text-uppercase">Select Smart View</label>
                            <v-select label="view_name" :options="views" @input="selectView" v-model="view"></v-select>
                        </div>
                    </div>
                ...
                <div class="divtbody  fit-divt-content">
                    <div class="divtbody-row" v-for="(record, i) in records.data" :key="record.id" :class="[(active_row.id == record.record_id)?'expended':'']">
                        ...
                        <div class="divtbody-elem wf-200">
                            <!-- <router-link  to="/fivenine" class="side-menu-link">  -->
                            <router-link :to="'prospects/' + record.record_id" class=""><b>{{ record.first_name }} {{ record.last_name }} </b></router-link>
                        ...
                        <div class="divtbody-elem  wf-120">
                            ...
                            <span v-if="record.last_agent" v-title="record.last_agent">
                                <i class="bi bi-person-fill"></i>
                            </span>
                            <span v-else><i class="bi bi-person"></i></span>
                        </div>
                        <div class="divtbody-elem  wf-120">
                            <span v-if="record.outreach_touched_at" v-title="myDateFormat('Touched', record.outreach_touched_at)"><i class="text-icon">TD</i></span>
                            <span v-if="record.outreach_created_at" v-title="myDateFormat('Created', record.outreach_created_at)"><i class="text-icon">CD</i></span>
                            <span v-if="record.last_agent_dispo_time" v-title="myDateFormat('Last Called ', record.last_agent_dispo_time)"><i class="text-icon">CT</i></span>
                        </div>
                    </div>
                </div>
                <div class="divtfoot">
...
export default {
    ...
    methods: {
        ...
        makingTestRecords(){
            for(var i = 0; i < this.recordContainer.length; i++){    
                ...
                if(this.allProspects[this.recordContainer[i]].stage_data){
                    var stage = this.allProspects[this.recordContainer[i]].stage_data.name;
                }else{
                    var stage = null;
                }              
        ...
        getOutreachData(pno) {
            ...
            this.$Progress.start();            
            this.form.post('/api/get-outreach-all-prospects-dashboard').then((response) => {                
        ...
        startDownload(){
            ...
            for(var i = 0; i < this.recordContainer.length; i++){
                ...
                let stage = '';
                if(this.allProspects[this.recordContainer[i]].stage_data){
                    stage = this.allProspects[this.recordContainer[i]].stage_data.name;
