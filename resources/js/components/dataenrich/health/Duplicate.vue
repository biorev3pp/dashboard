            ...
            <div class="dh-heading-sort">
                <div class="row m-0">
                    <div class="col-10 pl-0">
                        <h5 class="dh-heading-title">Groups with Duplicate Data</h5>
                    </div>
                ...
                <div class="dh-body-items" v-else-if="qtype == 'or'">
                    <div class="dhb-item-group" v-for="(dd, dk) in ddata" :key="'ddata'+dk" :class="[(active_data_key == dk)?'active':'']">
                        <div class="dhb-item-group-heading">
                            {{ dd.field | capitalize }} ({{ dd.data.length | freeNumber }}) 
                            
                            <i @click="active_data_key = dk" class="bi" :class="[(active_data_key == dk)?'bi-dash-circle':'bi-plus-circle']"></i>
                            <i @click="showProspect(dd)" class="bi bi-arrow-right-circle mr-2" v-show="active_data_key == dk"></i>
                        </div>
                        <div class="dhb-item-group-content">
                            <p v-for="(dvalue, dvk) in dd.data" :key="'dval'+dvk">
                                <span v-if="dd.dfield == 'mobilePhones' || dd.dfield == 'workPhones' || dd.dfield == 'homePhones'" v-title="dvalue[dd.dfield]">
                                    {{ dvalue[dd.dfield] }}
                                </span>
                                <span v-else>
                                     {{ (dvalue[dd.dfield])?dvalue[dd.dfield]:'[Empty]' }}
                                </span>
                                <b class="float-right">{{ dvalue.count }}</b>
                            </p>
                        </div>
                    </div>
                </div>
                <div class="dh-body-items" v-else>
                    <div class="dhb-item-group" v-for="(dd, dk) in ddata.data" :key="'ddata'+dk" :class="[(active_data_key == dk)?'active':'']">
                        <div class="dhb-item-group-heading">
                            ...
                            <i @click="active_data_key = dk" class="bi" :class="[(active_data_key == dk)?'bi-dash-circle':'bi-plus-circle']"></i>
                            <i @click="showAndProspect(dd)" class="bi bi-arrow-right-circle mr-2" v-show="active_data_key == dk"></i>
                        </div>
                        <div class="dhb-item-group-content">
            ...
            <div v-else>
                <div class="dh-contents-action">
                    ...
                    <div class="slide-action" :class="[(showaction)? 'slidein-action':'slideout-action']">
                        ...
                        <div class="action-btns" v-if="action_status === ''">
                            <div class="action-btn" @click="action_status = 'deleteBtn'; getCallEmailStatus();">
                                <i class="bi bi-x-octagon"></i>
                                <span> Delete Data </span>
                            </div>
                            <div class="action-btn" @click="action_status = 'mergeBtn'; getCallEmailStatus();">
                            ...
                            <div class="action-content p-2">
                                <div class="pt-2" v-if="action_status == 'deleteBtn'">
                                    <div class="alert alert-warning">
                                        ...
                                        <p class="fs-14">By this action, Corresponding records will be deleted from database.<br> And you will not be able to revert it at this moment.</p>
                                ...
                                <div class="pt-2"  v-else-if="action_status == 'mergeBtn'">
                                    <div v-if="selectedProspect.length >= 2">
                                        <span v-if="gce">
                                            <div class="text-center p-4">
                                        ...
                                        <span v-else>
                                            ...
                                            <p v-show="selectedProspectPrimary" class="text-right" v-else>
                                                <button class="btn btn-primary theme-btn" type="button" @click="mergeProspects">Start Merging</button>
                                            </p>
                                        </span>
                                    </div>
                                ...
                                <div class="pt-2" v-else>
                                    <p class="text-danger">No Action defined</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="divtable border-top">
                    ...
                    <div class="divtbody table-without-search">
                        <div class="divtbody-row" v-for="(record, rid) in filteredData" :key="'pdata-'+rid" :class="[(duplicateValues.indexOf(record[active_field])%2 == 0)?'op1':(duplicateValues.indexOf(record[active_field])%2 == 1)?'op2':'']">
...
export default {
    ...
    computed: {
        duplicateValues() {
            ...
            if(this.active_data) {
                let alldata = this.active_data.data;
                let ak = this.active_field
                return Array.from(new Set(alldata.map(( obj ) => {return obj[ak]} )));
            } else {
                return [];
            }
    ...
    watch: {
        ddata(newValue, oldValue) {
            if(newValue != oldValue) {
                ...
                this.search_sort_filter_data = {};
                this.selectedProspect = []
                this.selectedProspectData = []
    ...
    filters: {
        formatPhoneNumber(phoneNumberString) {
            ...
            var cleaned = ('' + phoneNumberString).replace(/\D/g, '').replace('+1', '').replace('+91', '').replace(' ', '');
    ...
    methods: {
        ...
        showProspect(dd) {
            this.active_data = dd;
            this.active_field = dd.dfield;
            this.mloader = true;
            axios.post('/api/get-related-prospects', {data:dd.data, active_field:dd.dfield}).then((response) => {
        ...
        showAndProspect(dd) {
            this.active_data = dd;
            this.mloader = true;
            axios.post('/api/get-related-and-prospects', {data:dd}).then((response) => {
                this.pdata = response.data
                this.updateFilterOptions()
                this.mloader = false
            })
        ...
        resetDateAction() {
            ...
            axios.post('/api/merge-selected-prospects', {data:this.selectedProspect}).then((response) => {
                this.running_Action = false;
            })
            if(this.qtype == 'or') {
                this.showProspect(this.active_data)
            } else {
                this.showAndProspect(this.active_data)
            }
        },
        mergeProspects() {
            this.running_Action = true;
            axios.post('/api/merge-selected-prospects', {data:this.selectedProspect, primary_data:this.selectedProspectPrimary}).then((response) => {
                this.running_Action = false;
            })
            if(this.qtype == 'or') {
                this.showProspect(this.active_data)
